...
public class UserDao {
    ...
    public boolean XoaDHNull(String idBan) {
        Connection cn = con.getJDBCConnection();
        String sql = "delete from tblDonHang where maBanNo = ? and thanhToan = 'k'";
        try {
            PreparedStatement preparedStatement = cn.prepareStatement(sql);
            preparedStatement.setString(1, idBan);
            int rs = preparedStatement.executeUpdate();
    ...
    //Begin: Chuyen Ban
    public void ThayDoiMaBanNo(String idDH, String maBan) {
        Connection cn = con.getJDBCConnection();
        String sql = "update tblDonHang set maBanNo = ? where maDH = ?";
        try {
            PreparedStatement preparedStatement = cn.prepareStatement(sql);
            preparedStatement.setString(1, maBan);
            preparedStatement.setString(2, idDH);
            preparedStatement.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(UserDao.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public boolean ChuyenBan(String idBanHienTai, String idBanChuyen) {
        Connection cn = con.getJDBCConnection();
        String sql = "";
        PreparedStatement preparedStatement = null;
        try {
            if(!daCoDH(idBanChuyen)) {
                System.out.println("chuaco");
                sql = "update tblDonHang set maBanNo = ? where maDH = ?";
                preparedStatement = cn.prepareStatement(sql);
                preparedStatement.setString(1, idBanChuyen);
                preparedStatement.setString(2, idDHbyIDBan(idBanHienTai));
                //ThayDoiMaBanNo(idDHbyIDBan(idBanHienTai), idBanChuyen);
            } else {
                System.out.println(idBanHienTai + idBanChuyen);
                
                MonTrung(idBanHienTai, idBanChuyen);
                System.out.println("2");
                sql = "update tblChiTietDonHang set idDH = ? where idDH = ?";
                preparedStatement = cn.prepareStatement(sql);
                preparedStatement.setString(1, idDHbyIDBan(idBanChuyen));
                preparedStatement.setString(2, idDHbyIDBan(idBanHienTai));
                //ThayDoiMaBanNo(idDHbyIDBan(idBanHienTai), idBanChuyen);
            }
            ...
            int rs = preparedStatement.executeUpdate();
                if(rs != -1) {
                    IsOrdered(idBanChuyen);
                    IsOrdered_k(idBanHienTai);
                    return true;
                }
    ...
    public boolean MonTrung(String idBanHienTai, String idBanChuyen)  {
        Connection cn = con.getJDBCConnection();
        String idDHBanHienTai = idDHbyIDBan(idBanHienTai);
        String idDHBanChuyen = idDHbyIDBan(idBanChuyen);
        List<String> listSPBanHienTai = listMon(idDHBanHienTai);
        List<String> listSPBanChuyen = listMon(idDHBanChuyen);
        String sql = "update tblChiTietDonHang set SLMua = SLMua + ? where idDH = ? and idSP = ?";
        PreparedStatement preparedStatement = null;
        int slThem = 0;
        
        //
        System.out.println("listt");
        for(String item : listSPBanHienTai) {
            System.out.println(item);
        }
        
        for(String item : listSPBanChuyen) {
            System.out.println(item);
        }
        //
        
        for(String mon : listSPBanHienTai) {
            System.out.println(mon);
            if(listSPBanChuyen.contains(mon)) {
                
            System.out.println(listSPBanChuyen.contains(mon));
                try {
                    slThem = slMon(idDHBanHienTai, mon);
                    System.out.println(slThem);
                    preparedStatement = cn.prepareStatement(sql);
                    preparedStatement.setInt(1, slThem);
                    preparedStatement.setString(2, idDHBanChuyen);
                    preparedStatement.setString(3, mon);
                    preparedStatement.executeUpdate();
                    XoaMonTrung(idDHBanHienTai, mon);
                } catch (SQLException ex) {
                    Logger.getLogger(UserDao.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public boolean XoaMonTrung(String idDH, String idSP) {
        Connection cn = con.getJDBCConnection();
        String sql = "delete from tblChiTietDonHang where idDH = ? and idSP = ?";
        try {
            PreparedStatement preparedStatement = cn.prepareStatement(sql);
            preparedStatement.setString(1, idDH);
            preparedStatement.setString(2, idSP);
            int resultSet = preparedStatement.executeUpdate();
            System.out.println("MonTrungXoa");
            if(resultSet != -1)
                return true;
        } catch (SQLException ex) {
            Logger.getLogger(UserDao.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public int slMon(String idDH, String idSP) {
        Connection cn = con.getJDBCConnection();
        String sql = "select SLMua from tblChiTietDonHang where idDH = ? and idSP = ?";
        try {
            PreparedStatement preparedStatement = cn.prepareStatement(sql);
            preparedStatement.setString(1, idDH);
            preparedStatement.setString(2, idSP);
    ...
    public List<String> listMon(String idDH) {
        List<String> listIDMon = new ArrayList<>();
        Connection cn = con.getJDBCConnection();
        String sql = "select idSP from tblChiTietDonHang where idDH = ?";
        try {
            PreparedStatement preparedStatement = cn.prepareStatement(sql);
            preparedStatement.setString(1, idDH);
            ResultSet resultSet = preparedStatement.executeQuery();
            while(resultSet.next())
                listIDMon.add(resultSet.getString("idSP"));
            return listIDMon;
        } catch (SQLException ex) {
            Logger.getLogger(UserDao.class.getName()).log(Level.SEVERE, null, ex);
        }
